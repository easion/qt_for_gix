...
{
    if (window){
		remove_gles_window(window);
#ifdef QT_BUILD_OPENGL_LIB //delete gles window
#endif
                               ...
                               int x, int y, uint w, uint h, long style)
{
	...
	gi_set_window_background(id, background,GI_BG_USE_NONE);

	printf("qt XCreateSimpleWindow: id = %ld/ parent = %ld [%d,%d,%d,%d]\n",id,parent,x,y,w,h);
    return id;
}
...
void QWidgetPrivate::updateX11AcceptFocus()
{
	...
	gi_set_window_attribute(q->internalWinId(), G_WINOW_ATTR_INPUTHINT, 
		q->testAttribute(Qt::WA_X11DoNotAcceptFocus) ? TRUE : FALSE);
...
#define Q_WIDGET_WITH_DND_FLAGS 0x10
		...
		if (q->testAttribute(Qt::WA_X11DoNotAcceptFocus))
		{
			style |= GI_FLAGS_NOFOCUS;
		}
    ...
    if (!initializeWindow) {
		...
		//gi_bool_t input = q->testAttribute(Qt::WA_X11DoNotAcceptFocus) ? FALSE : TRUE;

		gi_set_window_attribute(id, G_WINOW_ATTR_INPUTHINT, q->testAttribute(Qt::WA_X11DoNotAcceptFocus) ? TRUE : FALSE);
		
		if (qApp->d_func()->argv)
		{
			gi_set_command(id, qApp->d_func()->argv, qApp->d_func()->argc);
		}
...
void QWidgetPrivate::setGeometry_sys(int x,int y,int w,int h,bool  isMove)
{
	...
	//qDebug()<<"setGeometry_sys(start) "<<q<<" fid"<< fId<< " " <<x<<" "<<y<<" "<<w<<" "<<h<<" "<<isMove;    		
	...
	if (q->isWindow()){
        if (w == 0 || h == 0) {
            ...
            if (data.winid){
				gi_move_window( q->internalWinId(), x, y);
                gi_resize_window( q->internalWinId(),  w, h);
				resize_gles_window(q->internalWinId(),  w, h);
				//qDebug()<<"setGeometry_sys() "<<q<<" "<<x<<" "<<y<<" "<<w<<" "<<h<<" ";    		
            }	
			setConstraints_sys();//do_size_hints(q, extra);
            show_sys();
        } else {
			if (isMove){	
				//qDebug()<<"setGeometry_sys(move_window) "<<q<<" "<<x<<" "<<y<<" ";    		
				gi_move_window( q->internalWinId(), x, y);
      		}
			if (isResize){	
				//qDebug()<<"setGeometry_sys(resize_window) "<<q<<" "<<w<<" "<<h<<" ";   
				gi_resize_window( q->internalWinId(),  w, h);
				resize_gles_window(q->internalWinId(),w, h);
			}
        }
		...
		if (isResize) {
			//qDebug()<<"setGeometry_sys(resize_not_window) "<<q<<" "<<w<<" "<<h<<" ";    		
			gi_resize_window( q->internalWinId(),  w, h);
			resize_gles_window(q->internalWinId(),  w, h);
		}
		if (isMove)
		{
			//qDebug()<<"setGeometry_sys(move_not_window) "<<q<<" "<<x<<" "<<y<<" ";  		
			gi_move_window( q->internalWinId(), x, y);
		}
	}
...
QWindowSurface *QWidgetPrivate::createDefaultWindowSurface_sys()
{
    return new QGixWindowSurface(q_func());
}
#endif
    ...
    if (isWindow()) {
        ...
        if ((oldstate & Qt::WindowFullScreen) != (newstate & Qt::WindowFullScreen)) {
			...
			//if (support_fullscreen_state) {
				...
				//printf("gi_fullscreen_window got for %ld,%ld!!!", this->internalWinId(),this->winId());

				gi_fullscreen_window(this->winId(), (newstate & Qt::WindowFullScreen) == Qt::WindowFullScreen);
...
void QWidgetPrivate::setNetWmWindowTypes()
{
    ...
    if (!q->isWindow()) {
        if (q->internalWinId())
            gi_delete_property( q->internalWinId(), ATOM(_NET_WM_WINDOW_TYPE));
        return;
    }
    ...
    if (!windowTypes.isEmpty()) {
        gi_change_property( q->winId(), ATOM(_NET_WM_WINDOW_TYPE), GA_ATOM, 32,
                        G_PROP_MODE_Replace, (unsigned char *) windowTypes.constData(),
                        windowTypes.count());
    } else {
        gi_delete_property( q->winId(), ATOM(_NET_WM_WINDOW_TYPE));
    }
}
