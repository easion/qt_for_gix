...
#elif defined(Q_WS_GIX)
	if (!d->inputContext) {
		const QStringList inputMethods = QInputContextFactory::keys();
        // Try hbim and coefep first, then try others.
		QInputContext *qic = NULL;
		QApplication *that = const_cast<QApplication *>(this); 
        
		if (inputMethods.size() > 2 && inputMethods.contains(QLatin1String("imsw-multi"))) {
			qic = QInputContextFactory::create(QLatin1String("imsw-multi"), that);	
		}
		else{
			qic = QInputContextFactory::create(QLatin1String("Maliit"), that);		
		}

              
        if(!qic)qic = QInputContextFactory::create(QLatin1String("gix"), that);		
        that->d_func()->inputContext = qic;
    }
#elif defined(Q_OS_SYMBIAN)
    if (!d->inputContext) {
