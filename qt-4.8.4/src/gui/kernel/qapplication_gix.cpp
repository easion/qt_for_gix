...
#ifndef QT_NO_IM
#include "../inputmethod/qgixinputcontext_p.h"
#endif
    ...
    if (event->type == GI_MSG_MOUSE_MOVE) { // mouse move
        ...
        case GI_BUTTON_WHEEL_RIGHT:
            ...
            if (event->type == GI_MSG_BUTTON_DOWN){                
                ...
                bool hor = (((btn == GI_BUTTON_WHEEL_UP || btn == GI_BUTTON_WHEEL_DOWN) 
...
int QApplication::pfProcessEvent(gi_msg_t* event)
{
	...
	//#error "QT_BUILD_OPENGL_LIB ok!"
		#endif
		remove_gles_window(event->ev_window);
		//return 0;
		break;
...
#ifndef QT_NO_IM
		//int ime_status ;
		//gi_window_id_t target_window ;
		switch (event->params[0]) {
			case GI_STRUCT_IME_SETTARGE:
			//target_window = event->params[1];
			//ime_status = event->params[2];
			QGixInputContext::translateIMEvent(keywidget, static_cast<gi_msg_t*>(event));
			break;
			case GI_STRUCT_IME_OPEN:
				QGixInputContext::translateIMInitEvent(static_cast<gi_msg_t*>(event));
				break;
			case GI_STRUCT_IME_CLOSE:
				//if (keywidget) // should always exist
            //QGixInputContext::translateIMQueryEvent(keywidget, static_cast<gi_msg_t*>(event));
				break;
			default:
	...
	case GI_MSG_PAINT_AREA:
		//printf("GI_MSG_PAINT_AREA id=%d\n",event->body.message[0]);
		break;
