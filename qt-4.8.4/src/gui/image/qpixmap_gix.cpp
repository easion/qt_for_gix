...
gi_image_t *
QPixmap::toGixNativeImage(gi_image_t *orig) const
{
	gi_image_t *bitmap = NULL;
	gi_format_code_t format = (gi_format_code_t)GI_RENDER_a8r8g8b8;
	...
	if (orig ){
		if(GI_RENDER_FORMAT_BPP(orig->format) != GI_RENDER_FORMAT_BPP(format)){
			qDebug() << "toGixNativeImage: bad format" << orig->format;
			return 0;
		}
		bitmap = orig;
		format = (gi_format_code_t)orig->format;
		bytes_per_line = orig->pitch;
	}
    ...
    if (data->classId() == QPixmapData::RasterClass) {
		...
		if (orig && (w != orig->w || h != orig->h))
		{
			qDebug() << "toGixNativeImage: bad size";
			return 0;
		}

        const QImage image = d->image.convertToFormat(QImage::Format_ARGB32);
        //if(!bytes_per_line)bytes_per_line = w * 4;         
        ...
        return QPixmap(data).toGixNativeImage(bitmap);
    }        	
	return bitmap;
}
...
QPixmap
QPixmap::CopyGixNativePixmap(gi_window_id_t src)
{
	gi_image_t *img = NULL;
	gi_window_info_t wininfo;
	int err;	

	err = gi_get_window_info(src,&wininfo); //
	if (err<0){
		return QPixmap();
	}
	if (wininfo.res[2] == 0){
		...
		//共享模式
		img = gi_image_create_for_pixmap(src);
	}
	if (!img)
		  return QPixmap();
	return CopyGixNativeImage(img,true);
}

QPixmap
QPixmap::RefGixNativeImage(gi_image_t *bmp)
{
	int w, h;
	...
	w = bmp->w;
	h = bmp->h;
	int bytes_per_line =bmp->pitch;
    uchar *pixels = (uchar *)bmp->rgba;
	//QImage image(pixels,w,h,bytes_per_line,QImage::Format_ARGB32);
	QImage image(pixels,w,h,QImage::Format_ARGB32);
	return QPixmap::fromImage(image);
}

QPixmap
QPixmap::CopyGixNativeImage(gi_image_t *bmp, bool auto_free)
{
	int w, h;
                               ...
                               bool isMini)
{
    ...
    gi_image_t* hIcon;
	hIcon = pm.toGixNativeImage(NULL);
    return hIcon;
}
